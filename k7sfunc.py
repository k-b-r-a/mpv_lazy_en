...
def FMT_CHANGE(
    ...
    output = resizer(clip=input, width=w_out if w_out else None, height=h_out if h_out else None, filter_param_a=param_a,
                     filter_param_b=param_b, format=fmt_pix_val if fmt_pix >= 0 else None, dither_type=dither_val)
...
def FMT_CTRL(
    ...
    fmt_mpv = [vs.YUV420P8, vs.YUV420P10, vs.YUV422P8, vs.YUV422P10,
        ...
        if fmt_out == fmt_in:
            ...
            if (fmt_out not in fmt_safe) and (fmt_in in fmt_safe):
                ...
                clip = core.resize.Bicubic(
                    clip=input, width=w_in, height=h_in, filter_param_a=spl_b, filter_param_b=spl_c, format=fmt_out)
            else:
    ...
    if not h_max and not fmt_pix:
        ...
        if h_max >= h_in:
            ...
            output = core.resize.Bicubic(
                clip=clip, width=w_ds, height=h_ds, filter_param_a=spl_b, filter_param_b=spl_c)
...
def EQ(
    ...
    if hue is not None or sat is not None:
        ...
        output = core.std.ShufflePlanes(clips=[input, dst_u, dst_v], planes=[
                                        0, 0, 0], colorfamily=fmt_cf_in)
...
def LAYER_HIGH(
    ...
    if blur_m == 0:
        ...
        blur = core.std.Convolution(
            clip=cut0, matrix=[1, 1, 1, 1, 1, 1, 1, 1, 1])
        blur = core.std.Convolution(
            clip=blur, matrix=[1, 1, 1, 1, 1, 1, 1, 1, 1])
        output_blur = core.std.Convolution(
            clip=blur, matrix=[1, 1, 1, 1, 1, 1, 1, 1, 1])
...
def LINE_MASK(
    ...
    if cpu:  # r13+
        output = core.tcanny.TCanny(
            clip=input, sigma=1.5, t_h=8.0, t_l=1.0, mode=0, op=1, planes=plane)
    else:  # r12
        output = core.tcanny.TCannyCL(
            clip=input, sigma=1.5, t_h=8.0, t_l=1.0, mode=0, op=1, device=gpu, planes=plane)
...
def FPS_CTRL(
    ...
    if fps_in > fps_max:
        if fps_ret:
            ...
            output = FPS_CHANGE(input=input, fps_in=fps_in,
...
def CUGAN_NV(
    ...
    plg_dir = os.path.dirname(core.trt.Version()["path"]).decode()
    mdl_fname = ["pro-no-denoise3x-up2x", "pro-conservative-up2x",
                 "pro-denoise3x-up2x"][[-1, 0, 3].index(nr_lv)]
    ...
    output = core.resize.Bilinear(
        clip=cut2, format=fmt_in, matrix_s="709", range=1 if colorlv == 0 else None)
...
def EDI_US_STD(
    ...
    if cpu:
        ...
        if not hasattr(core, "nnedi3cl"):
            raise ModuleNotFoundError(
                f"模块 {func_name} 依赖错误：缺失插件，检查项目 nnedi3cl")
    ...
    if LooseVersion(nnedi3_resample.__version__) < LooseVersion("2"):
        raise ImportError(
            f"模块 {func_name} 依赖错误：缺失脚本 nnedi3_resample 的版本号过低，至少 2")
...
def ESRGAN_DML(
    ...
    mdl_fname = ["RealESRGANv2-animevideo-xsx2", "realesr-animevideov3", "animejanaiV2L1", "animejanaiV2L2", "animejanaiV2L3",
                 "animejanaiV3-HD-L1", "animejanaiV3-HD-L2", "animejanaiV3-HD-L3"][[0, 2, 5005, 5006, 5007, 5008, 5009, 5010].index(model)]
...
def NGU_HQ(
    ...
    mad_param = ["upscale(newWidth=%d,newHeight=%d,algo=nguAaHigh)" % (
        w_rs, h_rs), "setOutputFormat(format=yuv420,bitdepth=10)"]
...
def WAIFU_DML(
    ...
    plg_dir = os.path.dirname(core.ort.Version()["path"]).decode()
    mdl_pname = {3: "upconv_7_anime_style_art_rgb/",
                 5: "upresnet10/", 6: "cunet/"}.get(model)
    mdl_fname = ["scale2.0x_model", "noise0_scale2.0x_model", "noise1_scale2.0x_model",
                 "noise2_scale2.0x_model", "noise3_scale2.0x_model"][[-1, 0, 1, 2, 3].index(nr_lv)]
...
def MVT_LQ(
    ...
    if recal:
        cut_b = core.mv.Recalculate(
            super=cut_s, vectors=cut_b, thsad=200, blksize=blk_size / 2, search=2, searchparam=1)
        cut_f = core.mv.Recalculate(
            super=cut_s, vectors=cut_f, thsad=200, blksize=blk_size / 2, search=2, searchparam=1)
    else:
        cut_b, cut_f = cut_b, cut_f

    if block:
        output = core.mv.BlockFPS(
            clip=cut1, super=cut_s, mvbw=cut_b, mvfw=cut_f, num=fps_out * 1e6, den=1e6)
    else:
        output = core.mv.FlowFPS(
            clip=cut1, super=cut_s, mvbw=cut_b, mvfw=cut_f, num=fps_out * 1e6, den=1e6, mask=1)
    if w_tmp + h_tmp > 0:
...
def MVT_STD(
    ...
    cut_b = core.mv.Analyse(super=cut_s, blksize=64, searchparam=0, pelsearch=3,
                            isb=True, lambda_=0, lsad=10000, overlapv=16, badrange=0, search_coarse=4)
    cut_f = core.mv.Analyse(super=cut_s, blksize=64, searchparam=0, pelsearch=3,
...
def MVT_POT(
    ...
    bv = core.mv.Analyse(sup, blksize=blocksize, isb=True,
                         chroma=True, search=3, searchparam=2)
    fv = core.mv.Analyse(sup, blksize=blocksize, isb=False,
                         chroma=True, search=3, searchparam=2)
    use_block = clip.width > max_flow_width or clip.height > max_flow_height

    if use_block:
        output = core.mv.BlockFPS(clip, sup, bv, fv, num=target_num, den=target_den,
                                  mode=3, thscd1=th_block_diff, thscd2=th_block_changed)
    else:
        output = core.mv.FlowFPS(clip, sup, bv, fv, num=target_num, den=target_den,
...
def MVT_MQ(
    ...
    analParams = {'overlap': 0, 'overlapv': 0, 'search': search, 'dct': 0, 'truemotion': True,
                  ...
                  'divide': 0}
    bofp = {'thscd1': thscd1, 'thscd2': thscd2,
            'blend': True, 'num': int(fps_out * 1e6), 'den': 1e6}
    ...
    if block == True:
        output = core.mv.BlockFPS(
            cut0, sup, bvec, fvec, **bofp, mode=block_mode)
    else:
...
def RIFE_STD(
    ...
    cut2 = core.rife.RIFE(clip=cut1, model=(model+1) if t_tta else model, factor_num=fps_num, factor_den=fps_den,
                          gpu_id=gpu, gpu_thread=gpu_t, sc=True if sc_mode else False, skip=skip, skip_threshold=stat_th)
    output = core.resize.Bilinear(
        clip=cut2, format=fmt_in, matrix_s="709", range=1 if colorlv == 0 else None)
...
def RIFE_NV(
    ...
    if ext_proc:
        ...
        fin = vsmlrt.RIFE(clip=cut1, multi=fractions.Fraction(fps_num, fps_den), scale=scale_model, model=model, ensemble=t_tta, _implementation=2, video_player=True, backend=vsmlrt.BackendV2.TRT(
            ...
            device_id=gpu, short_path=True))
    output = core.resize.Bilinear(
        clip=fin, format=fmt_in, matrix_s="709", range=1 if colorlv == 0 else None)
    if not fps_factor.is_integer():
        output = core.std.AssumeFPS(
            clip=output, fpsnum=fps_in * fps_num * 1e6, fpsden=fps_den * 1e6)
...
def RIFE_NV_MVT(
    ...
    core.num_threads = vs_t
    w_in, h_in = output.width, output.height
    blk_size = 80
    w_tmp = math.ceil(w_in / blk_size) * blk_size - w_in
    h_tmp = math.ceil(h_in / blk_size) * blk_size - h_in
    if w_tmp + h_tmp > 0:
        cut0 = core.std.AddBorders(clip=output, right=w_tmp, bottom=h_tmp)
    else:
        cut0 = output
...
def SVP_LQ(
    ...
    smooth_param = "{rate:{num:%d,den:1,abs:false},algo:21,gpuid:%d,mask:{area:100},scene:{mode:0,limits:{m1:1800,m2:3600,scene:5200,zero:100,blocks:45}}}" % (
        fps_num, gpu)
    ...
    if clip.format.id == vs.YUV420P8:
        ...
        clip = core.resize.Bilinear(
            clip=clip, format=vs.YUV420P10, dither_type="random")
        clip8 = core.resize.Bilinear(clip=clip, format=vs.YUV420P8)

    svps = core.svp1.Super(clip8, super_param)
    svpv = core.svp1.Analyse(
        svps["clip"], svps["data"], clip if acc else clip8, analyse_param)
    output = core.svp2.SmoothFps(clip if acc else clip8, svps["clip"], svps["data"],
                                 svpv["clip"], svpv["data"], smooth_param, src=clip if acc else clip8, fps=fps_in)
...
def SVP_STD(
    ...
    smoothfps_params = "{rate:{num:%f,den:1000000,abs:true},algo:21,gpuid:%d,mask:{area:100},scene:{limits:{m1:1800,m2:3600,scene:5200,zero:100,blocks:45}}}" % (
        fps_out, gpu)
    ...
    vectors = core.svp1.Analyse(
        super["clip"], super["data"], input if acc else clip_f, analyse_params)
    smooth = core.svp2.SmoothFps(input if acc else clip_f, super["clip"], super["data"], vectors["clip"],
                                 vectors["data"], smoothfps_params, src=input if acc else clip_f, fps=fps_in)
    output = core.std.AssumeFPS(
...
def SVP_HQ(
    ...
    sp = "{gpu:%d}" % (acc)
    ap = "{block:{w:32,h:16,overlap:%d},main:{levels:5,search:{type:4,distance:-12,coarse:{type:4,distance:-1,trymany:true,bad:{range:0}}},penalty:{lambda:3.33,plevel:1.33,lsad:3300,pzero:110,pnbour:50}},refine:[{thsad:400},{thsad:200,search:{type:4,distance:-4}}]}" % (
        overlap)
    fp = "{gpuid:%d,algo:23,rate:{num:%d,den:%d,abs:true},mask:{cover:80,area:30,area_sharp:0.75},scene:{mode:0,limits:{scene:6000,zero:100,blocks:40}}}" % (
        gpu, round(min(max(target_fps, fps * 2, freq / 2), freq)) * 1000, 1001)
    ...
    def _svpflow(clip, fps, sp, ap, fp):
        ...
        r = *r, v["clip"], v["data"]
        clip = core.svp2.SmoothFps(
            clip if acc else clip8, *r, fp, src=clip, fps=fps)
        return clip
...
def SVP_PRO(
    ...
    analyse_param = "{vectors:3,block:{w:32,h:32,overlap:1},main:{levels:%d,search:{type:4,distance:3,sort:true,satd:false,coarse:{width:960,type:4,distance:4,satd:true,trymany:false,bad:{sad:1000,range:0}}},penalty:{lambda:3.0,plevel:1.3,lsad:8000,pnew:50,pglobal:50,pzero:100,pnbour:65,prev:0}},refine:[{thsad:800,search:{type:4,distance:1,satd:false},penalty:{pnew:50}}]}" % (
        ana_lv)
    smooth_param = "{rate:{num:%d,den:%d,abs:%s},algo:13,block:false,cubic:%d,gpuid:%d,linear:true,mask:{cover:40,area:16,area_sharp:0.7},scene:{mode:3,blend:false,limits:{m1:2400,m2:3601,scene:5002,zero:125,blocks:40},luma:1.5}}" % (
        fps_num, fps_den, multi, acc, gpu)
    ...
    if nvof:
        output = core.svp2.SmoothFps_NVOF(
            clip, smooth_nvof_param, nvof_src=clip8, src=clip, fps=fps_in)
    else:
        super = core.svp1.Super(clip8, super_param)
        vectors = core.svp1.Analyse(
            super["clip"], super["data"], clip if acc else clip8, analyse_param)
        output = core.svp2.SmoothFps(clip if acc else clip8, super["clip"], super["data"],
                                     vectors["clip"], vectors["data"], smooth_param, src=clip if acc else clip8, fps=fps_in)
...
def BILA_NV(
    ...
    if not hasattr(core, "bilateralgpu_rtc"):
        raise ModuleNotFoundError(
            f"模块 {func_name} 依赖错误：缺失插件，检查项目 bilateralgpu_rtc")
    ...
    if fmt_in == vs.YUV444P16:
        ...
        cut0 = core.resize.Bilinear(clip=input, format=vs.YUV444P16)
    cut1 = core.bilateralgpu_rtc.Bilateral(
        clip=cut0, sigma_spatial=nr_spat, sigma_color=nr_csp, device_id=gpu, num_streams=gpu_t, use_shared_memory=True)
    output = core.resize.Bilinear(clip=cut1, format=fmt_in)
...
def BM3D_NV(
    ...
    if not hasattr(core, "bm3dcuda_rtc"):
        raise ModuleNotFoundError(
            f"模块 {func_name} 依赖错误：缺失插件，检查项目 bm3dcuda_rtc")
    ...
    ref = core.bm3dcuda_rtc.BM3D(
        clip=cut0, sigma=nr_lv, block_step=bs_ref, device_id=gpu)
    cut1 = core.bm3dcuda_rtc.BM3D(
        clip=cut0, ref=ref, sigma=nr_lv, block_step=bs_out, device_id=gpu)
    output = core.resize.Bilinear(clip=cut1, format=fmt_in)
...
def CCD_STD(
    ...
    def _ccd(src: vs.VideoNode, threshold: float = 4) -> vs.VideoNode:
        ...
        ex_ccd = core.akarin.Expr([r, g, b, src],
                                  ...
                                  f'A@ {thr} < 1 0 ? B@ {thr} < 1 0 ? C@ {
                                      thr} < 1 0 ? D@ {thr} < 1 0 ? '
                                  f'E@ {thr} < 1 0 ? F@ {thr} < 1 0 ? G@ {
                                      thr} < 1 0 ? H@ {thr} < 1 0 ? '
                                  f'I@ {thr} < 1 0 ? J@ {thr} < 1 0 ? K@ {
                                      thr} < 1 0 ? L@ {thr} < 1 0 ? '
                                  f'M@ {thr} < 1 0 ? N@ {thr} < 1 0 ? O@ {
                                      thr} < 1 0 ? P@ {thr} < 1 0 ? '
                                  '+ + + + + + + + + + + + + + + 1 + Q! '
...
def DFTT_STD(
    ...
    if not hasattr(core, "dfttest2_cpu"):
        raise ModuleNotFoundError(
            f"模块 {func_name} 依赖错误：缺失插件，检查项目 dfttest2_cpu")
    ...
    if fmt_in == vs.YUV444P16:
        ...
        cut0 = core.resize.Bilinear(clip=input, format=vs.YUV444P16)
    cut1 = dfttest2.DFTTest2(clip=cut0, planes=plane, sigma=nr_lv, sbsize=size_sb,
                             sosize=size_so, tbsize=size_tb, backend=dfttest2.Backend.CPU())
    output = core.resize.Bilinear(clip=cut1, format=fmt_in)
...
def DFTT_NV(
    ...
    if not hasattr(core, "dfttest2_nvrtc"):
        raise ModuleNotFoundError(
            f"模块 {func_name} 依赖错误：缺失插件，检查项目 dfttest2_nvrtc")
    ...
    if fmt_in == vs.YUV444P16:
        ...
        cut0 = core.resize.Bilinear(clip=input, format=vs.YUV444P16)
    cut1 = dfttest2.DFTTest2(clip=cut0, planes=plane, sigma=nr_lv, sbsize=size_sb, sosize=size_so,
                             tbsize=size_tb, backend=dfttest2.Backend.NVRTC(device_id=gpu, num_streams=gpu_t))
    output = core.resize.Bilinear(clip=cut1, format=fmt_in)
...
def DPIR_NR_NV(
    ...
    if model == 0:
        ...
        cut1 = core.std.ShufflePlanes(
            clips=cut0, planes=0, colorfamily=vs.GRAY)
        cut2 = core.resize.Bilinear(
            clip=cut1, format=vs.GRAYH, matrix_in_s="709")
    else:
        cut2 = core.resize.Bilinear(
            clip=cut0, format=vs.RGBH, matrix_in_s="709")
    ...
    if model == 0:
        pre_mg = core.resize.Bilinear(clip=fin, format=fin.format.replace(
            bits_per_sample=fmt_bit_in, sample_type=0))
# pre_mg = core.resize.Bilinear(clip=fin, format=fmt_in, matrix_s="709", range=1 if colorlv==0 else None)
        output = core.std.ShufflePlanes(clips=[pre_mg, cut0, cut0], planes=[
                                        0, 1, 2], colorfamily=fmt_cf_in)
    else:
        output = core.resize.Bilinear(
            clip=fin, format=fmt_in, matrix_s="709", range=1 if colorlv == 0 else None)
...
def FFT3D_STD(
    ...
    if mode == 1:
        if not hasattr(core, "fft3dfilter"):
            raise ModuleNotFoundError(
                f"模块 {func_name} 依赖错误：缺失插件，检查项目 fft3dfilter")
    elif mode == 2:
        if not hasattr(core, "neo_fft3d"):
            raise ModuleNotFoundError(
                f"模块 {func_name} 依赖错误：缺失插件，检查项目 neo_fft3d")

    core.num_threads = vs_t

    if mode == 1:
        output = core.fft3dfilter.FFT3DFilter(
            clip=input, sigma=nr_lv, planes=plane, bt=frame_bk, ncpu=cpu_t)
    elif mode == 2:
        output = core.neo_fft3d.FFT3D(
            clip=input, sigma=nr_lv, planes=plane, bt=frame_bk, ncpu=cpu_t, mt=False)
...
def NLM_STD(
    ...
    if nlm_m == 1:
        ...
        if not hasattr(core, "nlm_ispc"):
            raise ModuleNotFoundError(
                f"模块 {func_name} 依赖错误：缺失插件，检查项目 nlm_ispc")
    ...
    if blur_m:
        ...
        if nlm_m == 1:
            ...
            cut1 = core.nlm_ispc.NLMeans(clip=blur, d=frame_num, a=rad_sw, s=rad_snw, h=nr_lv,
                                         channels="AUTO", wmode=2, wref=1.0, rclip=None)
    output = core.resize.Bilinear(
        clip=merge if blur_m else cut1, format=fmt_in)
...
def COLOR_P3W_FIX(
    ...
    cut = core.fmtc.primaries(clip=cut, prims="p3d65",
                              primd="p3dci", wconv=True)
    if linear:
        cut = core.fmtc.transfer(clip=cut, transs="linear", transd="1886")

    output = core.resize.Bilinear(
        clip=cut, format=vs.YUV420P8, matrix_s="709", range=1 if colorlv == 0 else None)
...
def CSC_RB(
    ...
    def _Levels(clip, input_low, gamma, input_high, output_low, output_high, coring=True):
        ...
        tvLow, tvHigh = _scale(16, peak), [
            _scale(235, peak), _scale(240, peak)]
        scaleUp, scaleDown = peak / _scale(219, peak), _scale(219, peak) / peak

        def _get_lut1(x):
            p = ((x - tvLow) * scaleUp - input_low) / \
                divisor if coring else (x - input_low) / divisor
            p = min(max(p, 0), 1) ** gamma * \
        ...
        def _get_lut2(x):
            q = _cround((x - neutral) * (output_high -
                        output_low) / divisor + neutral)
    ...
    def _GetPlane(clip, plane=0):
        ...
        last = core.std.ShufflePlanes(
            clips=clip, planes=plane, colorfamily=vs.GRAY)
        return last
    ...
    red = _Levels(area, _scale(255, peak), 1.0,
                  _scale(255, peak), _scale(255, peak), 0)
    ...
    mask = _Levels(mask, _scale(250, peak), 1.0,
                   _scale(250, peak), _scale(255, peak), 0)
    ...
    mask = _Levels(mask, _scale(10, peak), 1.0, _scale(
        10, peak), 0, _scale(255, peak)).std.Inflate()
    input_c = EQ(input.resize.Spline16(
        src_left=cx, src_top=cy), sat=sat_lv2, vs_t=vs_t)
    fu = core.std.MaskedMerge(clipa=_GetPlane(
        input, 1), clipb=_GetPlane(input_c, 1), mask=mask)
    fv = core.std.MaskedMerge(clipa=_GetPlane(
        input, 2), clipb=_GetPlane(input_c, 2), mask=mask)

    output = core.std.ShufflePlanes([input, fu, fv], planes=[
                                    0, 0, 0], colorfamily=fmt_cf_in)
...
def DEBAND_STD(
    ...
    if fmt_in == vs.YUV444P16:
        ...
        cut0 = core.resize.Bilinear(clip=input, format=vs.YUV444P16)
    output = core.neo_f3kdb.Deband(clip=cut0, range=bd_range, y=bdy_rth, cb=bdc_rth, cr=bdc_rth, grainy=grainy, grainc=grainc,
                                   sample_mode=spl_m, dynamic_grain=grain_dy, mt=True, keep_tv_range=True if color_lv == 1 else False, output_depth=depth)
...
def DEINT_STD(
    ...
    if deint_m == 1:
        ...
        if not hasattr(core, "yadifmod"):
            raise ModuleNotFoundError(
                f"模块 {func_name} 依赖错误：缺失插件，检查项目 yadifmod")
    elif deint_m == 3:
    ...
    if ref_m == 1:
        ...
        ref = core.nnedi3cl.NNEDI3CL(
            clip=input, field=3 if tff else 2, dh=False, device=gpu)
    elif ref_m == 3:
        ref = core.eedi3m.EEDI3CL(
            clip=input, field=3 if tff else 2, dh=False, device=gpu)

    if deint_m == 1:
        output = core.bwdif.Bwdif(
            clip=input, field=3 if tff else 2, edeint=ref)
    elif deint_m == 2:
        output = core.yadifmod.Yadifmod(
            clip=input, edeint=ref, order=1 if tff else 0, mode=1)
    elif deint_m == 3:
        output = core.tdm.TDeintMod(
            clip=input, order=1 if tff else 0, mode=1, length=6, ttype=0, edeint=ref)
...
def DEINT_EX(
    ...
    output = qtgmc.QTGMCv2(input=input, fps_in=fps_in, obs=obs, deint_lv=deint_lv,
                           src_type=src_type, deint_den=deint_den, tff=tff, cpu=cpu, gpu=gpu)
...
def DPIR_DBLK_NV(
    ...
    plg_dir = os.path.dirname(core.trt.Version()["path"]).decode()
    mdl_fname = ["drunet_deblocking_grayscale",
                 "drunet_deblocking_color"][[2, 3].index(model)]
    mdl_pth = plg_dir + "/models/dpir/" + mdl_fname + ".onnx"
    if not os.path.exists(mdl_pth):
        raise vs.Error(f"模块 {func_name} 所请求的模型缺失")
...
def EDI_AA_STD(
    ...
    output = core.resize.Spline36(
        clip=clip, width=w_in, height=h_in, src_left=-0.5, src_top=-0.5)
...
# raise vs.Error(f"模块 {func_name} 的子参数 plane 的值无效")
    ...
    output = core.eedi2cuda.AA2(clip=input, mthresh=10, lthresh=20, vthresh=20,
                                estr=2, dstr=4, maxd=24, map=0, nt=50, pp=1, num_streams=gpu_t, device_id=gpu)
...
def IVTC_STD(
    ...
    if fps_in <= 24 or fps_in >= 31 or (fps_in >= 26 and fps_in <= 29):
        ...
        if ivtc_m == 1:
            ...
            cut0 = core.std.AssumeFPS(
                clip=input, fpsnum=fps_in * 1e6, fpsden=1e6)
...
def STAB_STD(
    ...
    inter = core.std.Interleave([core.rgvs.Repair(temp, input.focus2.TemporalSoften2(
        1, threshold, threshold, 25, 2), mode=[1]), input])
...
def STAB_HQ(
    ...
    def _scdetect(clip: vs.VideoNode, threshold: float = 0.1) -> vs.VideoNode:
        ...
        if clip.format.color_family == vs.RGB:
            sc = clip.std.ModifyFrame(
                clips=[clip, sc], selector=_copy_property)
        return sc
    ...
    def _Stab(clp, dxmax=4, dymax=4, mirror=0):
        ...
        inter = core.std.Interleave([core.rgvs.Repair(temp, _average_frames(
            clp, weights=[1] * 3, scenechange=25 / 255), mode=[1]), clp])
...
def UAI_DML(
    ...
    be_param = vsmlrt.BackendV2.ORT_DML(
        device_id=gpu, num_streams=gpu_t, fp16=True)
    infer = vsmlrt.inference(
        clips=clip, network_path=mdl_pth, backend=be_param)
    output = core.resize.Bilinear(
...
# raise vs.Error(f"模块 {func_name} 的子参数 res_opt 或 res_max 的值无效")
    ...
    clip = core.resize.Bilinear(
        clip=input, format=vs.RGBH if fp16 else vs.RGBS, matrix_in_s="709")
    if clamp:
        clip = core.akarin.Expr(clips=clip, expr="x 0 1 clamp")
...
def UVR_MAD(
        ...
        rca_lv: typing.Literal[0, 1, 2, 3, 4, 5,
    ...
    colorlv = getattr(input.get_frame(0).props, "_ColorRange", 0)
    fmt_in = input.format.id
    fmt_mad = [vs.YUV420P8, vs.YUV420P10, vs.YUV420P16, vs.YUV422P8, vs.YUV422P10,
               vs.YUV422P16, vs.YUV440P8, vs.YUV444P8, vs.YUV444P10, vs.YUV444P16]
    algo = (["nguAa", "nguSoft", "nguStandard", "nguSharp"][ngu - 1] +
            ["Low", "Medium", "High", "VeryHigh"][ngu_q - 1]) if ngu > 0 else None
    w_rs, h_rs = w_in * 2, h_in * 2
    quality = ["low", "medium", "high", "veryHigh"][rca_q - 1]

    param_size1 = ("upscale(newWidth=%d,newHeight=%d,algo=%s,sigmoidal=on)" % (
        w_rs, h_rs, algo)) if algo is not None else None
    param_other2 = ("rca(strength=%d,quality=%s)" %
                    (rca_lv, quality)) if rca_lv > 0 else None
